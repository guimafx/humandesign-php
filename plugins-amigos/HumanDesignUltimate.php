...
private function calculatePrecise(array $b):array
{
...
private function calculateApproximate(array $b):array
{
    ...
    return [
        'approximate'=>true,
        'type'=>$this->stableValue($charts,'type'),
        'profile'=>$this->stableValue($charts,'profile'),
        'authority'=>$this->stableValue($charts,'authority'),
        'variables'=>'Varia conforme horário',
        'phs'=>'Pode variar',
        'have'=>$charts[0]['have']
    ];
}

private function stableValue($charts,$field)
{
    $vals=array_column($charts,$field);
    return count(array_unique($vals))==1
        ? $vals[0]
        : 'Varia conforme horário';
}
...
private function ephemeris($jd,$planet,$lat,$lon)
{
    if(function_exists('swe_calc_ut')){
        ...
        $map=['SUN'=>SE_SUN,'NORTH_NODE'=>SE_TRUE_NODE];
...
public function renderMandala($chart,$size=720)
{
    ...
    for($i=0;$i<64;$i++){
        ...
        $svg.="<text x='$x' y='$y'
        font-size='10'
        text-anchor='middle'>{$this->raveOrder[$i]}</text>";
    }
    ...
    if(!empty($chart['approximate'])){
        $svg.="<text x='20' y='120' fill='red' font-size='14'>
        ⚠ Horário de nascimento não informado.
        Este chart é aproximado.
        </text>";
    }
